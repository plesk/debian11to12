# The tool to dist-upgrade servers with Plesk from Debian 11 to 12

## Introduction
This utility is the official tool to dist-upgrade servers with Plesk from Debian 11 to 12.
...
```shell
> wget https://github.com/plesk/debian11to12/releases/download/v1.0.4/debian11to12-1.0.4.zip
> unzip debian11to12-1.0.4.zip
...
```shell
> screen -r debian11to12
```

You can also call debian11to12 in the background:
```shell
> ./debian11to12 &
```
...
Following the preliminary checks, the tool proceeds with the dist-upgrade process, which is divided into multiple stages. Some stages end with a reboot. You can check the list of stages and steps by `./debian11to12 --show-plan`.
...
The dist-upgrader writes its log to the `/var/log/plesk/debian11to12.log` file, as well as to stdout.
...
## Issue handling
If for some reason the process has failed, inspect the log. By default, it's put to `/var/log/plesk/debian11to12.log`. If the process was interrupted before the first reboot, you can restart it with the `--resume` option. If the problem has happened after the first reboot, you can restart the process by running `systemctl restart plesk-dist-upgrader`.

If something goes wrong, you will be informed on the next login with this message:
```
===============================================================================
Message from Plesk dist-upgrader tool:
Something went wrong during dist-upgrade by debian11to12.
See the /var/log/plesk/debian11to12.log file for more information.
You can remove this message from the /etc/motd file.
===============================================================================
```

### Send feedback
If you got any error, please [create an issue on github](https://github.com/plesk/debian11to12/issues). Describe your problem and attach the feedback archive or at least the log to the issue. The feedback archive can be created by calling the tool with the `--prepare-feedback` option:
```shell
> ./debian11to12 --prepare-feedback
```
